...
class Trainer_Base():
    ...
    def __init__(self, args, command='constant', kfold_idx=0, **kwargs):
        
        print(args)

        if 'command' in args:
            command = args.command
        if 'kfold_idx' in args:
        ...
        if 'train' in os.listdir(args.data_path):
            self.dataset, self.dataset_val = self.prepare_datasets(args, **kwargs)
        else:
            if command == 'skfold':
                dataset, skfold_indices_list = self.prepare_datasets_cls(args, command=command, fold_idx=kfold_idx, n_splits=args.kfold_number, **kwargs)
                self.dataset = torch.utils.data.Subset(dataset, skfold_indices_list[kfold_idx][0])
                self.dataset_val = torch.utils.data.Subset(dataset, skfold_indices_list[kfold_idx][1])
                
            else:
                self.dataset, self.dataset_val = self.prepare_datasets_cls(args, **kwargs)
...
class Trainer_lite(Trainer_Base):
    ...
    @staticmethod
    def prepare_datasets_cls(args, split_ratio=0.8, command='random', n_splits=5, shuffle=False, random_state=None, **kwargs):
        """ 
            cls, classes, assume the directory's hierarchy is like subclasses folders 
            
            1. shuffle: naive process, just randomly split the train and val datasets
            2. constant: keep the order of images, usually used for feature extraction or other tasks need to keep their arrangement.
            3. kfold: for k-fold train-and-validation, if use kfold, return <dataset> and <indices>
        """
        ...
        elif command == 'skfold':     # k-fold must be used with external command
    ...
    def prepare_dataloaders(self, args):
        ...
        collate_fn = None
        if isinstance(self.dataset, torchvision.datasets.folder.ImageFolder):
            self.num_classes = len(self.dataset.classes) 
        elif isinstance(self.dataset, torch.utils.data.dataset.Subset):
