...
def SFP_pruning_parser(parser):
    ...
    parser.add_argument('--prune_rate_distance', type=float, default=0.5)
...
class SFP_Mask_Base:
            ...
            if index in self.mask_index:
                ...
                if hasattr(module, 'bias') and (getattr(module, 'bias') is not None):
                    ...
                    if isinstance(module, (nn.Conv2d, nn.Linear)):
                        ...
                        bias_mask = torch.Tensor(torch.sum(mask.reshape(mask.shape[0], -1), dim=1)==0).to(torch.int)     # --- one dimensional mask
                ...
                if remove_grad and (module.weight.grad is not None):
                    ...
                    if hasattr(module, 'bias') and (getattr(module, 'bias') is not None):
                        ...
                        module.bias.grad.data = module.bias.grad.data * bias_mask
        ...
        for index, (name, module) in enumerate(self.model.named_modules()):     # --- for each layer
            ...
            if index in self.mask_index:
                ...
                if isinstance(module, (nn.Conv2d, nn.Linear)):
                    ...
                    self.mask_dict[index], filter_index = self.get_mask_by_norm(module.weight.data, index, return_idx=True)
    ...
    def set_pruning_layer_and_ratio(self, args) -> None:
        ...
        if 'vgg' in args.arch:
            ...
            if args.prune_classifier:

                for index in [_ for _, (_n, _m) in enumerate(self.model.named_modules()) if isinstance(_m, nn.Linear)][:-1]:
    ...
    def get_mask_by_norm(self, input, index, return_idx=False) -> torch.Tensor:
        
        filter_index = self.get_indices_by_norm(input, self.prune_rate[index])
        
        mask = self.get_mask(input, filter_index)
        
        return mask, filter_index if return_idx else mask
    ...
    def get_indices_by_norm(self, input, prune_rate, p=2, **kwargs) -> torch.Tensor:
        
        filter_pruned_num = int(input.shape[0] * prune_rate)
        sum_of_kernel = torch.norm(input.view(input.shape[0], -1), p=p, dim=1)
        
        return torch.argsort(sum_of_kernel)[:filter_pruned_num]

    
    def count_zero_weights(self, log=None, **kwargs) -> None:
        ...
        for idx, (name, module) in enumerate(self.model.named_modules()):
            ...
            if idx in self.mask_dict:
                ...
                layer_count = f"layer: {idx}, pct_zero is {pct_zero*100:.2f} %, number of nonzero weight is {cnz}, zero is {cz}"
...
class ASFP_Mask(SFP_Mask_Base):
    ...
    def mask_initialization(self, args, epoch=0, **kwargs) -> None:
        ...
        return self.dynamic_pruning_rate[epoch]
...
class FPGM_Mask(SFP_Mask_Base):
            ...
            if index in self.mask_index_distance:
                ...
                if isinstance(module, (nn.Conv2d, nn.Linear)):
                    ...
                    filter_index_distance_reduced = (
                                                    self.get_indices_by_distance(target_tensor, index, allow_intersection)
                                                    if not allow_intersection and (filter_index is not None)
                                                    else self.get_indices_by_distance(module.weight.data, index)
                                                    )
    ...
    def get_indices_by_distance(self, input, index, allow_intersection:bool) -> torch.Tensor:
        """
            ...
            intuitively, a lot of 'similarity' metrics can be used here
        """

        similar_pruned_num = input.shape[0] * self.prune_rate_distance[index]
        
        if not allow_intersection:
            similar_pruned_num = int(similar_pruned_num * (1/(1-self.prune_rate[index])))
        else:
            similar_pruned_num = int(similar_pruned_num)
        ...
        def _apply(_mask):
            ...
            if hasattr(module, 'bias') and (getattr(module, 'bias') is not None):
                ...
                if isinstance(module, (nn.Conv2d, nn.Linear)):
                    ...
                    bias_mask = torch.Tensor(torch.sum(_mask.reshape(_mask.shape[0], -1), dim=1)==0).to(torch.int)     # --- one dimensional mask
...
class AFPGM_Mask(FPGM_Mask):
    ...
    def mask_initialization(self, args, epoch=0, **kwargs) -> None:
        ...
        return self.dynamic_pruning_rate[epoch], self.dynamic_pruning_rate_distance[epoch]
